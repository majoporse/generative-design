...
function draw() {
  ...
  //     offset =
  //       noise(i * 0.01, k * 0.01, millis() / 10000) * OFFSET - OFFSET / 2;
  //     offsets.push(offset);

  //     curveVertex(k * POINT_WIDTH, i * RADIUS + RADIUS / 2 + offset);
  //   }
      ...
      for (let y = 0; y < cur_floor[0].length; y++) {
        ...
        if (cur_height >= lower_bound && cur_height < upper_bound) {
          ...
          for (let n of neighbors) {
            if (n > upper_bound) {
              console.log(
                `{${x}, ${y}} edge, cur_height: ${cur_height}, neighbor: ${n}, bounds: [${lower_bound}, ${upper_bound}]`
              );
              cur_floor[x][y] = 1;
              break;
            }
